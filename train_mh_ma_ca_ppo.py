...
def categorize_actions(actions):  # pylint: disable=C0116

    actions_categories_dict = {'CDUCAB': ['cdu-cabinet-1', 'cdu-cabinet-2', 'cdu-cabinet-3',
                                          'cdu-cabinet-4', 'cdu-cabinet-5'],
                               'CT': ['cooling-tower-1']}
    categorized_actions = {}
    for agent_id, action in actions.items():
        if agent_id == 'CDUCAB':
            for i, action_category in enumerate(actions_categories_dict[agent_id]):
                categorized_actions[action_category] = {'top-level': action['top-level'][i],
                                                        'valve-level': action['valve-level'][i]}
        else:
...
def train():
    ...
    # print avg reward in the interval (in num timesteps)10
    print_freq = max_ep_len * 5
    # log avg reward in the interval (in num timesteps)5
    log_freq = max_ep_len * 5
    # save model frequency (in num timesteps)
    save_model_freq = int(2e3)
    best_reward = {'CDUCAB': float('-inf'),
                   'CT': float('-inf')}
    # initialize best reward as negative infinity
    ...
    reward_fn_version = 2
    # reward_fn_version  # change this to prevent overwriting weights in same env_name folder
    run_num_pretrained = 2
    # version of exogensous variable iterator to use; possible choices for v are 1 and 2
    exogen_gen_v = 2

    env = MH_SmallFrontierModel(
        use_reward_shaping=f'reward_shaping_v{reward_fn_version}', exogen_gen_v=exogen_gen_v)
    print("Using Reward Shaping Version : ", env.env.use_reward_shaping)
    ...
    log_f_name = log_dir + '/MH_MA_CA_PPO_' + \
        env_name + "_log_" + str(run_num) + ".csv"
    ...
    checkpoint_path = {}
    for agent_id, _ in agent_mdp_dict.items():
        checkpoint_path[agent_id] = directory + "PPO_{}_{}_{}_agent_{}.pth".format(
            env_name, random_seed, run_num_pretrained, agent_id)
    ...
    for agent_id, agent_info in agent_mdp_dict.items():
        ...
        print("--------------------------------------------------------------------------------------------")
        if agent_info.get('has_continuous_action_space', True):
            print("Initializing a continuous action space policy")
            print(
                "--------------------------------------------------------------------------------------------")
            print("starting std of action distribution : ",
                  agent_info['action_std_init'])
    ...
    for agent_id, _ in agent_mdp_dict.items():
        if os.path.exists(checkpoint_path[agent_id]):
            print("loading pretrained weights for agent : " + agent_id)
            ppo_agent.load(checkpoint_path[agent_id], agent_id)
    ...
    while time_step <= max_training_timesteps:
        ...
        cabinet_totalepisode_reward = {'cdu-cabinet-1': 0, 'cdu-cabinet-2': 0,
                                       'cdu-cabinet-3': 0, 'cdu-cabinet-4': 0, 'cdu-cabinet-5': 0}

        for _ in range(1, max_ep_len+1):

            # list to store actions for each agent
            actions = {'CDUCAB': {'top-level': None,
                                  'valve-level': None}, 'CT': None}

            # select action for CDU setups
            action = ppo_agent.select_action(
                batch_state_dict['CDUCAB'], 'CDUCAB')
            ...
            if time_step % action_std_decay_freq == 0:
                for agent_id, _ in agent_mdp_dict.items():
                    ...
                    if agent_mdp_dict[agent_id].get('has_continuous_action_space', True):
                        ppo_agent.decay_action_std(
                            action_std_decay_rate, min_action_std, agent_id)

            # log in logging file
            if time_step % log_freq == 0:

                # log average reward till last episode for each agent
                log_avg_reward['CDUCAB'] = log_running_reward['CDUCAB'] / \
                    log_running_episodes
                log_avg_reward['CDUCAB'] = round(log_avg_reward['CDUCAB'], 4)

                log_avg_reward['CT'] = log_running_reward['CT'] / \
                    log_running_episodes
            ...
            if time_step % print_freq == 0:
                ...
                print_avg_reward['CDUCAB'] = print_running_reward['CDUCAB'] / \
                    print_running_episodes
                print_avg_reward['CDUCAB'] = round(
                    print_avg_reward['CDUCAB'], 2)

                print_avg_reward['CT'] = print_running_reward['CT'] / \
                    print_running_episodes
                ...
                for agent_id, _ in agent_mdp_dict.items():
                    ...
                    if print_avg_reward[agent_id] > best_reward[agent_id]:
                        ...
                        print(
                            "--------------------------------------------------------------------------------------------")
                        print(
                            f"saving model for agent {agent_id} at : " + checkpoint_path[agent_id])
        ...
        writer.add_scalar('Episode Reward CDUCAB',
                          current_ep_reward['CDUCAB'], i_episode)
        writer.add_scalar('Episode Reward CT',
                          current_ep_reward['CT'], i_episode)
